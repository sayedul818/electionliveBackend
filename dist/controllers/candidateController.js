...
export const listCandidatesBySeat = async (req, res) => {
...
export const upsertCandidate = async (req, res) => {
    ...
    const candidate = await CandidateModel.findOneAndUpdate({ _id: payload._id }, { $set: payload }, { upsert: true, new: true });
    await recomputeSeatWinner(payload.seatId);
    return res.json(candidate);
};
export const setCandidateVotes = async (req, res) => {
    const seatId = Number(req.params.seatId);
    ...
    const candidate = await CandidateModel.findOneAndUpdate({ _id: candidateId, seatId }, { $set: { votes: Math.max(0, votes) } }, { new: true });
    if (!candidate) {
...
export const syncCandidatesFromLive = async (req, res) => {
    const seatId = Number(req.params.seatId);
    ...
    for (const row of rows) {
        ...
        const candidate = await CandidateModel.findOneAndUpdate({ seatId, name }, {
            $set: {
                name,
                seatId,
                partyLabel,
    ...
    return res.json(upserted.map((candidate) => ({
        ...candidate.toObject(),
        id: candidate._id.toString(),
    })));
};
